...
FeatureConfig fcfg;
    ...
    // Final evaluation run: log ONLY the best-found policy to a clean Gantt CSV
    GanttLogger logger(string("gantt_") + instance + string(".csv"));
    {
        Simulation sim(base_jobs, base_machines, build_rules(), feats, &logger);
        uint64_t seq_base = 999000000ULL; // separate deterministic stream for final log
        (void)sim.run_episode(hh.gbest, hh, seq_base, /*eps*/ 0.0);
        logger.write_sorted();
    }
    cout << "Rules: ";
    {
