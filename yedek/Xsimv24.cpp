...
// Adds exploration via epsilon-greedy and tiny tie-breaking noise.
    // Select rule index using given weights (particle policy) + state scaling
    // Adds exploration via epsilon-greedy and tiny tie-breaking noise.
    int select_rule(const vector<double>& w, const StateVector& s, uint64_t seed, double eps) const {
        uint64_t x = seed ^ 0xD1B54A32D192ED03ULL;

        // 1) Exploration: with probability eps, pick a random rule
...
while (ai < argc) {
        ...
        if (key == "--eps0") {
            ...
            cout << "Usage: xsim.exe [instance] [--eps0 <v>] [--epsmin <v>] [--iters <n>] [--swarm <n>] [--seed <n>]\n";
            return 0;
        } else {
...
FeatureConfig fcfg;
    ...
    // Training phase: no Gantt logging (avoid mixing multiple episodes)
        ...
        // Epsilon schedule: high exploration at start, decays to p.eps_min
        double frac = (p.iters<=1) ? 1.0 : (1.0 - (double)iter / (double)(p.iters-1));
        double current_eps = max(p.eps_min, p.eps0 * frac);
        ...
        for (int i=0;i<p.swarm_size;++i) {
            ...
            const auto& w = hh.particle_weights(i);
            // Use a deterministic tie-break stream per particle to reduce evaluation noise.
            // (Varying seq_base across iterations can change simultaneous-dispatch ordering and add noise.)
            uint64_t seq_base = (uint64_t)i*1000ULL;

            SimResult res = sim.run_episode(w, hh, seq_base, current_eps);
            hh.update_fitness(i, res.Cmax);
    ...
    // Final deterministic evaluation run (eps=0) + clean Gantt CSV
    const uint64_t final_seq_base = 999000000ULL;
    GanttLogger logger(string("gantt_") + instance + string(".csv"));
    SimResult final_res;
    {
        Simulation sim(base_jobs, base_machines, build_rules(), feats, &logger);
        final_res = sim.run_episode(hh.gbest, hh, final_seq_base, /*eps*/ 0.0);
        logger.write_sorted();
    }
