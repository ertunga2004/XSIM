...
static RuleGroup group_of_rule_index(int k) {
    ...
    // 4 FIFO (FIFO_OTHER)
    // 5 SIO  (PROC_TIME)     [next-op time]
    // 6 PT+WINQ (PROC_TIME)  [look-ahead proxy]
    if (k==0 || k==1 || k==5 || k==6) return RuleGroup::PROC_TIME;
...
while (ai < argc) {
        ...
        if (key == "--eps0") {
            ...
            cout << "Usage: xsim.exe [instance] [--eps0 <v>] [--epsmin <v>] [--iters <n>] [--swarm <n>] [--seed <n>] [--evalk <n>] [--finalk <n>] [--fitavg]\n";
            return 0;
        } else {
...
FeatureConfig fcfg;
    ...
    // Training phase: no Gantt logging (avoid mixing multiple episodes)
        ...
        for (int i=0;i<p.swarm_size;++i) {
            ...
            for (int rep=0; rep<eval_k; ++rep) {
                ...
                SimResult r = sim.run_episode(w, hh, seq_base, current_eps);
                if (fitness_take_min) {
...

    {
        ...
        for (int rep=0; rep<final_k; ++rep) {
            uint64_t seq_base =
                ...
                999000000ULL ^
                ((uint64_t)rep * 97ULL);
            SimResult r = sim.run_episode(hh.gbest, hh, seq_base, /*eps*/ 0.0);
    ...
    {
        Simulation sim(base_jobs, base_machines, build_rules(), feats, &logger);
        final_res = sim.run_episode(hh.gbest, hh, best_seq, /*eps*/ 0.0);
        logger.write_sorted();
    }

    cout << "Done. Best Cmax=" << (long long)final_res.Cmax
         << " (train_best=" << (long long)hh.gbest_fit << ", best_seq=" << (unsigned long long)best_seq << ")\n";
    {
        auto r = build_rules();
