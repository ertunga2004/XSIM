...
static bool schedule_tabu_search_critical_blocks(
    ...
    // tabu on (machine,swap_index) for adjacent swap at position i (swap i and i+1)
    unordered_map<long long,int> tabu_until;
    auto key_of = [&](int m, int i)->long long { return ((long long)m<<32) ^ (unsigned long long)(i); };
    ...
    for(int it=0; it<ts_iters; ++it){
        ...
        int bestCandM = -1, bestCandI = -1;
        bool found = false;

        auto consider_swap = [&](int m, int i){
            if(i < 0 || i+1 >= (int)order_by_m[m].size()) return;
            long long k = key_of(m,i);
            bool is_tabu = false;
        ...
        // mark reverse move tabu too (same adjacent swap index is symmetric)
        tabu_until[key_of(bestCandM, bestCandI)] = it + max(1, tabu_tenure);
...
static bool improve_gantt_with_schedule_ls(
    ...
    bool ok = false;
if(ts_iters > 0){
    ok = schedule_tabu_search_critical_blocks(J,M,machine_of,proc,order_by_m,ts_iters,tabu_tenure,&bestC,&bestStart);
} else {
...
while (ai < argc) {
        ...
        if (key == "--eps0") {
            ...
            cout << "Usage: xsim.exe [instance] [--eps0 <v>] [--epsmin <v>] [--iters <n>] [--swarm <n>] [--seed <n>] [--evalk <n>] [--finalk <n>] [--fitavg] [--sgs gt|event] [--lsiters <n>] [--lsstep <v>] [--traindet] [--slsiters <n>] [--tsiters <n>] [--tabu <tenure>]\n";
            return 0;
        } else {
...
// Final deterministic re-evaluation (eps=0):
    ...
    SimResult final_res;
    {
        ...
        if (sls_iters > 0 || ts_iters > 0) {
            ...
            bool ok = improve_gantt_with_schedule_ls(base_jobs, (int)base_jobs.size(), (int)base_machines.size(), logger.rows, sls_iters, ts_iters, tabu_tenure, &newC);
