...
std::unique_ptr<Rule> create_rule_by_name(const std::string& rule_name) {
    if (rule_name == "SPT") {
        return std::make_unique<RuleSPT>();
    }
    if (rule_name == "LPT") {
        return std::make_unique<RuleLPT>();
    }
    if (rule_name == "MWKR") {
        return std::make_unique<RuleMWKR>();
    }
    if (rule_name == "MOR") {
        return std::make_unique<RuleMOR>();
    }
    if (rule_name == "FIFO") {
        return std::make_unique<RuleFIFO>();
    }
    if (rule_name == "SIO") {
        return std::make_unique<RuleSIO>();
    }
    if (rule_name == "PT+WINQ") {
        return std::make_unique<RulePTWINQ>();
    }
    return nullptr;
}
...
std::vector<std::unique_ptr<Rule>> build_rules() {
    ...
    for (const auto& name : build_rule_names()) {
        rules.emplace_back(create_rule_by_name(name));
    }
    return rules;
}
