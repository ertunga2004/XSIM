param(
    [string]$Matrix = ".\final_run_matrix.csv",
    ...
    [string]$FinalResults = ".\final_results.csv",
    [string]$AggregatedResults = ".\aggregated_results.csv"
)
...
if (-not (Test-Path $Matrix)) { throw "Final matrix bulunamadi: $Matrix" }
...
if ($matrixRows.Count -eq 0) { throw "Final matrix bos gorunuyor: $Matrix" }

$resultsHeader = @(
    'phase','evaluation_split','instance','preset_id','seed','runtime_budget_sec','termination_mode',
    'reference_value','reference_type','candidate_tier','run_id','run_dir','status','feasible','cmax',
    'gap_percent','runtime_sec','result_json','metadata_json','convergence_csv','schedule_csv','notes'
) -join ','
Set-Content -Path $FinalResults -Value $resultsHeader -Encoding UTF8

Write-Host "=========================================="
Write-Host "XSIM FINAL PIPELINE BASLADI"
...
foreach ($row in $matrixRows) {
    ...
    $candidateTier = $row.candidate_tier
    $evaluationSplit = $row.evaluation_split
    $notes = $row.notes
    ...
    Write-Host ("Tier     : {0}" -f $candidateTier)
    ...
    Add-ContentSafe -Path $FinalResults -Value $csvLine
}

# Aggregate final results
$finalRows = @(Import-Csv -Path $FinalResults)
$groups = $finalRows | Group-Object instance, preset_id

$aggHeader = @(
    'phase','instance','preset_id','reference_value','reference_type','candidate_tier',
    ...
    'success_rate','winning_flag','selected_final_winner','decision_note'
) -join ','
...
foreach ($g in $groups) {
    ...
    $candidateTier = $rows[0].candidate_tier
...
# Mark best mean-gap per instance as winning_flag=yes and selected_final_winner=yes
    ...
    if ($valid.Count -gt 0) {
        ...
        foreach ($s in $ig.Group) {
            if ($null -ne $s.mean_gap_percent -and [double]$s.mean_gap_percent -eq [double]$bestMeanGap) {
                $s.winning_flag = 'yes'
                $s.selected_final_winner = 'yes'
            }
            else {
                $s.winning_flag = 'no'
                $s.selected_final_winner = 'no'
            }
        }
    }
}
...
foreach ($s in $summaries) {
    $line = ConvertTo-CsvLine @(
        ...
        [string]$s.selected_final_winner,
...
Write-Host ""
Write-Host "=========================================="
Write-Host "XSIM FINAL PIPELINE BITTI"
Write-Host ("Final Results      : {0}" -f (Resolve-Path $FinalResults))
