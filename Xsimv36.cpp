...
// -------- Tabu Search on machine orders (critical-block adjacent swaps) --------
// Escapes local optima by allowing non-improving moves with a tabu list.
static bool schedule_tabu_search_critical_blocks(
    int J, int M,
    const vector<int>& machine_of,
    const vector<double>& proc,
    vector<vector<int>>& order_by_m,
    ...
    if(ts_iters <= 0) return false;
    const int N = J*M;
    SchEval cur = eval_machine_order_schedule(J,M,machine_of,proc,order_by_m);
    if(!cur.acyclic) return false;

    double best = cur.Cmax;
    vector<vector<int>> best_order = order_by_m;
    vector<double> best_start = cur.start;

    // tabu list (key -> iteration until which move is forbidden)
    unordered_map<long long,int> tabu_until;
    auto key_swap = [&](int m, int i)->long long {
        return (1LL<<62) ^ ((long long)m<<32) ^ (unsigned long long)(i); // tag=01
    };
    auto key_ins  = [&](int m, int i, int j)->long long {
        return (2LL<<62) ^ ((long long)m<<40) ^ ((long long)i<<20) ^ (unsigned long long)(j); // tag=10
    };
    ...
    for(int it=0; it<ts_iters; ++it){
        ...
        // Build position lookup on each machine
        vector<vector<int>> pos(M, vector<int>(N, -1));
        for(int m=0;m<M;m++){
            for(int i=0;i<(int)order_by_m[m].size();i++){
                pos[m][order_by_m[m][i]] = i;
        ...
        struct Block { int m; int lpos; int rpos; };
        vector<Block> blocks;
        for(size_t i=0;i<crit.size();){
            int u = crit[i];
            int m = machine_of[u];
            ...
            if(r - l + 1 >= 2) blocks.push_back(Block{m,l,r});
            i = j;
        }
        if(blocks.empty()){
            // Fallback: allow any machine adjacent swap on critical nodes' machines
            // (rare, but prevents stagnation)
            for(int u: crit){
                int m = machine_of[u];
                int p = pos[m][u];
                if(p>=0 && p+1 < (int)order_by_m[m].size())
                    blocks.push_back(Block{m,p,p+1});
            }
        }

        double bestCandC = 1e300;
        SchEval bestCandEval;
        int bestCandType = -1; // 0=swap, 1=insert
        int bestCandM = -1, bestCandI = -1, bestCandJ = -1;
        bool found = false;

        auto consider_swap = [&](int m, int i){
            if(i < 0 || i+1 >= (int)order_by_m[m].size()) return;
            long long k = key_swap(m,i);
            bool is_tabu = false;
            auto itT = tabu_until.find(k);
            if(itT != tabu_until.end() && itT->second > it) is_tabu = true;

            // apply swap in-place
            swap(order_by_m[m][i], order_by_m[m][i+1]);
            SchEval ev = eval_machine_order_schedule(J,M,machine_of,proc,order_by_m);
            // undo
            swap(order_by_m[m][i], order_by_m[m][i+1]);

            if(!ev.acyclic) return;

            // aspiration: if improves global best, allow even if tabu
            if(is_tabu && !(ev.Cmax + 1e-9 < best)) return;

            if(ev.Cmax < bestCandC){
                bestCandC = ev.Cmax;
                bestCandEval = std::move(ev);
                bestCandType = 0;
                bestCandM = m;
                bestCandI = i;
                bestCandJ = -1;
        ...
        auto consider_insert = [&](int m, int from, int to){
            // Move the operation at position "from" to position "to" (insertion).
            if(from < 0 || from >= (int)order_by_m[m].size()) return;
            if(to < 0) to = 0;
            if(to > (int)order_by_m[m].size()) to = (int)order_by_m[m].size();
            if(to == from || to == from+1) return; // no-op

            long long k = key_ins(m, from, to);

            bool is_tabu = false;
            auto itT = tabu_until.find(k);
            if(itT != tabu_until.end() && itT->second > it) is_tabu = true;

            auto& vec = order_by_m[m];
            int node = vec[from];
            vec.erase(vec.begin() + from);
            int to2 = to;
            if(from < to) to2 = to - 1;
            vec.insert(vec.begin() + to2, node);

            SchEval ev = eval_machine_order_schedule(J,M,machine_of,proc,order_by_m);

            // undo
            vec.erase(vec.begin() + to2);
            vec.insert(vec.begin() + from, node);
        ...
        for(const auto& b: blocks){
            if(tsmove_mode == 0 || tsmove_mode == 2){
                for(int i=b.lpos; i<=b.rpos-1; ++i) consider_swap(b.m, i);
                consider_swap(b.m, b.lpos-1);
                consider_swap(b.m, b.rpos);
            }
            if(tsmove_mode == 1 || tsmove_mode == 2){
                // Small but effective insertion moves on critical blocks
                consider_insert(b.m, b.lpos, b.rpos+1); // move first after last
                consider_insert(b.m, b.rpos, b.lpos);   // move last before first
                consider_insert(b.m, b.lpos, b.lpos-1); // move first before block
                consider_insert(b.m, b.rpos, b.rpos+2); // move last after block
            }
        }

        if(!found) break;

        // move to best candidate (even if worse than current)
        if(bestCandType == 0){
            swap(order_by_m[bestCandM][bestCandI], order_by_m[bestCandM][bestCandI+1]);
            cur = std::move(bestCandEval);
            tabu_until[key_swap(bestCandM, bestCandI)] = it + max(1, tabu_tenure);
        } else {
            auto& vec = order_by_m[bestCandM];
            int from = bestCandI;
            int to   = bestCandJ;
            if(to < 0) to = 0;
            if(to > (int)vec.size()) to = (int)vec.size();
            int node = vec[from];
            vec.erase(vec.begin() + from);
            int to2 = to;
            if(from < to) to2 = to - 1;
            ...
            cur = std::move(bestCandEval);
            tabu_until[key_ins(bestCandM, from, to)] = it + max(1, tabu_tenure);
        }

        if(cur.Cmax + 1e-9 < best){
            best = cur.Cmax;
    ...
    if(out_best_Cmax) *out_best_Cmax = best;
    if(out_best_start) *out_best_start = std::move(best_start);
    return true;
}
...
static bool improve_gantt_with_schedule_ls(
    ...
    if(sls_iters <= 0 && ts_iters <= 0) return false;
    const int N = J*M;
    ...
    bool ok = false;
if(ts_iters > 0){
    ok = schedule_tabu_search_critical_blocks(J,M,machine_of,proc,order_by_m,ts_iters,tabu_tenure,tsmove_mode,&bestC,&bestStart);
} else {
    ok = schedule_local_search_critical_blocks(J,M,machine_of,proc,order_by_m,sls_iters,&bestC,&bestStart);
}
if(!ok) return false;
...
int sls_iters = 0;      // schedule-level local search iterations (hill-climb, critical-block swaps)
int ts_iters  = 0;      // schedule-level TABU search iterations (critical-block swaps)
int tabu_tenure = 10;   // tabu tenure
int tsmove_mode = 2; // tabu move: 0=swap, 1=insert, 2=mixed
...
while (ai < argc) {
        ...
        if (key == "--eps0") {
            ...
            string v = need(key);
            if (!parse_int(v, ts_iters) || ts_iters < 0) { cerr << "Bad --tsiters value\n"; return 1; }
        } else if (key == "--tabu") {
            string v = need(key);
            if (!parse_int(v, tabu_tenure) || tabu_tenure < 0) { cerr << "Bad --tabu value\n"; return 1; }
        } else if (key == "--tsmove") {
            string v = need(key);
            if (v == "swap") tsmove_mode = 0;
            else if (v == "insert") tsmove_mode = 1;
            else if (v == "mixed") tsmove_mode = 2;
            else { cerr << "Bad --tsmove value (use swap|insert|mixed)\n"; return 1; }
        } else if (key == "--fitavg") {
            ...
            cout << "Usage: xsim.exe [instance] [--eps0 <v>] [--epsmin <v>] [--iters <n>] [--swarm <n>] [--seed <n>] [--evalk <n>] [--finalk <n>] [--fitavg] [--sgs gt|event] [--lsiters <n>] [--lsstep <v>] [--traindet] [--slsiters <n>] [--tsiters <n>] [--tabu <tenure>] [--tsmove swap|insert|mixed]\n";
            return 0;
        } else {
...
// Final deterministic re-evaluation (eps=0):
    ...
    SimResult final_res;
    {
        ...
        if (sls_iters > 0 || ts_iters > 0) {
            ...
            bool ok = improve_gantt_with_schedule_ls(base_jobs, (int)base_jobs.size(), (int)base_machines.size(), logger.rows, sls_iters, ts_iters, tabu_tenure, tsmove_mode, &newC);
